...
export function filter(rows, context, name, type, prefText) {
  const newRows = [];
...
export function filterByStats(row, context, name, type, prefText): boolean {
  const s = row.stats;
  ...
  if (
    ...
    if (!row.stats.find((s) => {
      s.name === 'hitpoints' &&
      s.type === 'stat' && s.min >= context.fields.hpm && s.type === 'stat' && s.positive === true
    })) {
      return false;
    }
  }
  ...
  if (context.fields.resist !== 0) {
    if (!row.stats.find((s) => {
      return s.type === 'resistance' && s.min >= context.fields.resist && s.positive === true && ((!prefText || prefText === '') || s.name === prefText);
    })) {
      return false;
    }
  }
